...
async function startConversation (client, seller){
    ...
    // get seller id
    const sellerId = await axios.get(process.env.REACT_APP_MESSAGES_API_URL + "/api/users/?username=" + seller)

    //check if a conversation if this seller already exist
    const convExist = await axios.get(process.env.REACT_APP_MESSAGES_API_URL + `/api/conversations/find/${client.id}/${sellerId.data._id}`);
    ...
    try {
        const res = await axios.post(process.env.REACT_APP_MESSAGES_API_URL + "/api/conversations/postConv/?senderId=" + client.id + "&receiverId=" + sellerId.data._id);
